...
\begin{itemize}
  \item \emph{none}: all features are included.  
  \item \emph{user}: all user-level features, including the user ID and other
    calculated features like entropy and measures of exercise bursts, are
    removed.
  \item \emph{userid} and \emph{user other}: only user ID or only the calculated
    features user features, respectively, are removed.
  \item \emph{word}: Token and token root IDs; previous, next, and dependency-tree
    root word IDs; and morphological, part of speech, and dependency tree
    features are removed.
  \item \emph{word id} and \emph{word other}: only word IDs or only other
    word features, respectively, are removed.
  \item \emph{neighbors}: Both word IDs and other word features are removed, but
    only for the previous, next, and dependency-tree root words. Information
    about the present word is maintained.
  \item \emph{external}: External information about the word, including corpus
    frequency, Levenshtein distance from translation, and age of acquisition,
    are removed.
  \item \emph{temporal}: Temporal information, including number and timing of
