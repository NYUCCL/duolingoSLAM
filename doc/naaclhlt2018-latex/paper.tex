...
data set. \hl{Anselm: if space permits, mention what the train, dev, and test data sets look like? e.g.:  80\% of each user's exercises are in train, 10\% in each of dev and test.} Once the model structure was finalized, we trained on the combined
...
\hl{Anselm: mention AUROC of baseline model for comparison? shall the reader be impressed by our performance. perhaps also mention the winner's score (the deep learning team) for commparison, to show that our score was not that much worse? `we were 0.01 behind the deep learning model' sounds cooler to me than `we were third'}
...
\hl{Anselm: I don't understand the last sentence here.}
...
\hl{I like that sentence! Bam take that, deep learning}
